...
def gaussain_map(size, center, sigma):
	x0,y0 = center
	yy,xx = np.mgrid[0:size, 0:size]
	g = np.exp(-((xx-x0) **2 + (yy-y0)**2) / (2 * sigma * sigma))
	return g

def add_noise_new(heatmap, channel, label, center_1, center_2, sigma, size):
	g1 = gaussain_map(size, center_1, sigma)

	if center_2[0] == -1:
		g = g1
	else:
		g2 = gaussain_map(size, center_2, sigma)
		g = np.maximum(g1, g2)
	
	if label[channel] == 1:
		heatmap[channel] += g
	else:
		heatmap[channel] += (g - 1.0)
...
class MyDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		if os.path.exists(image_path.replace('images','images_ori')):
			image_path = image_path.replace('images','images_ori')
